...
describe("downloadOutcomeLabels", () => {
  afterEach(() => {
    vi.unstubAllGlobals();
    vi.restoreAllMocks();
  });

  it("downloads the outcome-labels export attachment and returns the filename", async () => {
    const { downloadOutcomeLabels } = await import("./api-client");
    ...
    const fetchMock = vi.fn().mockResolvedValue({
      ...
      headers: {
        get: (name: string) =>
          name.toLowerCase() === "content-disposition"
            ? 'attachment; filename="aegis-AAPL-assessment-1-outcome-labels.json"'
            : null,
      },
      blob: async () => blob,
      json: async () => null,
    ...
    await expect(
      downloadOutcomeLabels("http://localhost:8000", "AAPL", 1, 20),
    ).resolves.toBe("aegis-AAPL-assessment-1-outcome-labels.json");

    expect(fetchMock).toHaveBeenCalledWith(
      "http://localhost:8000/research/AAPL/assessments/1/outcome-labels/export?limit=20",
      expect.objectContaining({ credentials: "include" }),
    );
    expect(click).toHaveBeenCalled();
    expect(revokeObjectURL).toHaveBeenCalledWith("blob:mock");
...
describe("downloadProbabilityCalibrations", () => {
  afterEach(() => {
    vi.unstubAllGlobals();
    vi.restoreAllMocks();
  });

  it("downloads the calibrations export attachment and returns the filename", async () => {
    const { downloadProbabilityCalibrations } = await import("./api-client");
    const blob = new Blob(["[]"], { type: "application/json" });
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
      status: 200,
      headers: {
        get: (name: string) =>
          name.toLowerCase() === "content-disposition"
            ? 'attachment; filename="aegis-AAPL-assessment-1-calibrations.json"'
            : null,
      },
      blob: async () => blob,
      json: async () => null,
    ...
    await expect(
      downloadProbabilityCalibrations("http://localhost:8000", "AAPL", 1, 20),
    ).resolves.toBe("aegis-AAPL-assessment-1-calibrations.json");

    expect(fetchMock).toHaveBeenCalledWith(
      "http://localhost:8000/research/AAPL/assessments/1/calibrations/export?limit=20",
      expect.objectContaining({ credentials: "include" }),
    );
    expect(click).toHaveBeenCalled();
    expect(revokeObjectURL).toHaveBeenCalledWith("blob:mock");
...
describe("downloadResearchAssessments", () => {
  ...
  it("downloads assessments JSON with component_source filter", async () => {
    const { downloadResearchAssessments } = await import("./api-client");
    const blob = new Blob(["[]"], { type: "application/json" });
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://localhost:8000/research/AAPL/assessments/export?limit=20&component_source=mixed",
      expect.objectContaining({ credentials: "include" }),
    );
  });
});
