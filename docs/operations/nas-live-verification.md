# NAS Live Verification Checklist (Phase 17)
...
| 8 | SSH `alembic current` (when SSH configured) | includes **`0008`** or `head` |
| 9 | TLS (if enabled) | HTTPS URLs + `AEGIS_SESSION_COOKIE_SECURE=true` |
...
Keep `AEGIS_RESEARCH_CALIBRATION_AFTER_LABEL_ENABLED=false` until Phase 16 readiness shows
