      ...
      <table className="min-w-full text-left text-sm">
        <thead className="border-b border-aegis-line text-aegis-muted">
          <tr>
            ...
            <th className="py-2 font-medium">Fetch fallback</th>
          </tr>
        </thead>
        <tbody>
          {bars.map((bar) => (
            <tr
              key={`${bar.source}-${bar.symbol}-${bar.trading_date}`}
              className="border-b border-aegis-line/70"
            >
              <td className="py-2 pr-4 font-mono">{bar.trading_date}</td>
              <td className="py-2 pr-4 font-mono">{bar.source}</td>
              ...
              <td className="py-2 pr-4">{bar.data_quality}</td>
              <td
                className="py-2 font-mono"
                data-testid={`daily-bar-fetch-fallback-${bar.source}-${bar.trading_date}`}
              >
                {bar.fetch_fallback ?? "—"}
              </td>
            </tr>
