...
AAPL often shows all four. The summary control did not show how many diagnostics
are active, so operators had to expand to know severity at a glance.
...
callout testids. Expose the count via ``evidence-labeling-diagnostics-active-count``.
