# NAS Live Verification Checklist (Phase 17 + Phase 21 + Phase 23 + Phase 25 + Phase 27 + Phase 29 + Phase 31)
...
| 2 | `GET {API}/ready` | 200 |
| 3 | Unauthenticated watchlist / daily-bars / research latest / **assessments list** / calibration-readiness / evidence-summary / evidence-summary/export | 401 |
...
| 8 | Authenticated `GET /research/{symbol}/assessments?limit=` | **200** JSON array (`[]` OK) |
| 9 | Authenticated `GET .../assessments/{id}/calibrations` and `.../outcome-labels` | **200** JSON array (`[]` OK) |
| 10 | Authenticated `GET /research/{symbol}/evidence-summary` | **200**, `state=research_only`; log present label keys and end-date keys only (none OK) |
| 11 | Authenticated `GET /research/{symbol}/evidence-summary/export` | **200**, `Content-Disposition` attachment, `state=research_only` |
| 12 | SSH `alembic current` (when SSH configured) | includes **`0008`** or `head` |
| 13 | TLS (if enabled) | HTTPS URLs + `AEGIS_SESSION_COOKIE_SECURE=true` |
