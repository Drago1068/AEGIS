...
function Get-HttpStatus {
    param([string]$Url)
    # Prefer curl.exe (Windows 10+) so 401/3xx do not throw like Invoke-WebRequest may.
    $curl = Get-Command curl.exe -ErrorAction SilentlyContinue
    if ($null -eq $curl) {
        throw "curl.exe is required for verify.ps1 (OpenSSH/curl ships with modern Windows)."
    }
    $code = & curl.exe -sS -o NUL -w "%{http_code}" --max-time 30 $Url
...
set -euo pipefail
cd '$dir'
out=`$(docker compose -f docker-compose.yml -f docker/nas/docker-compose.nas.yml --env-file .env.nas --project-directory . exec -T backend alembic current)
echo "`$out"
...
"@
    ...
    Write-Host "  docker compose -f docker-compose.yml -f docker/nas/docker-compose.nas.yml --env-file .env.nas logs --tail=200 backend"
    Write-Host "  docker compose -f docker-compose.yml -f docker/nas/docker-compose.nas.yml --env-file .env.nas logs --tail=200 frontend"
    Write-Host "  docker compose -f docker-compose.yml -f docker/nas/docker-compose.nas.yml --env-file .env.nas ps"
} else {
    Write-Host ""
...
Write-Host ""
Write-Host "Verification passed for HTTP checks against $api and $frontend."
