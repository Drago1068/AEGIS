...
async def _build_research_evidence_summary(
    ...
    *,
    ...
    stored_bar_calendar_lag_trading_days: int | None = None
    if snapshots:
        (
    ...
    most_recent_labeled_outcome_label_method_id = (
        most_recent_labeled_outcome_label.label_method_id
        if most_recent_labeled_outcome_label is not None
        else None
    )
    most_recent_labeled_outcome_label_method_version = (
        most_recent_labeled_outcome_label.label_method_version
        if most_recent_labeled_outcome_label is not None
        else None
    )
    most_recent_labeled_outcome_label_schema_version = (
        most_recent_labeled_outcome_label.schema_version
        if most_recent_labeled_outcome_label is not None
        else None
    )
    most_recent_labeled_outcome_label_state = (
        most_recent_labeled_outcome_label.state
        if most_recent_labeled_outcome_label is not None
        else None
    )
    most_recent_labeled_outcome_label_bar_source = (
        most_recent_labeled_outcome_label.bar_source
        if most_recent_labeled_outcome_label is not None
        else None
    )
    most_recent_labeled_outcome_label_computed_at = (
        most_recent_labeled_outcome_label.computed_at
        if most_recent_labeled_outcome_label is not None
        else None
    )
    most_recent_labeled_outcome_label_as_of_trading_date = (
        most_recent_labeled_outcome_label.as_of_trading_date
        if most_recent_labeled_outcome_label is not None
        else None
    )
    ...
    latest_lookback_start_date = (
        latest_assessment.lookback_start_date if latest_assessment is not None else None
    )
    ...
    latest_event_time = (
        latest_assessment.event_time if latest_assessment is not None else None
    )
    ...
    latest_outcome_label_id = (
        latest_outcome_label.id if latest_outcome_label is not None else None
    )
    ...
    latest_outcome_label_method_version = (
        latest_outcome_label.label_method_version
        if latest_outcome_label is not None
        else None
    )
    latest_outcome_label_schema_version = (
        latest_outcome_label.schema_version if latest_outcome_label is not None else None
    )
    latest_outcome_label_state = (
        latest_outcome_label.state if latest_outcome_label is not None else None
    )
    latest_outcome_label_bar_source = (
        latest_outcome_label.bar_source if latest_outcome_label is not None else None
    )
    latest_outcome_label_as_of_trading_date = (
        latest_outcome_label.as_of_trading_date
        if latest_outcome_label is not None
        else None
    )
    ...
    latest_calibration_horizon_key = (
        latest_calibration.outcome_horizon_key if latest_calibration is not None else None
    )
    latest_calibration_computed_at = (
        latest_calibration.computed_at if latest_calibration is not None else None
    )
    latest_calibration_corpus_count = (
        latest_calibration.corpus_count if latest_calibration is not None else None
    )
    latest_calibration_bucket_count = (
        latest_calibration.bucket_count if latest_calibration is not None else None
    )
    ...
    latest_calibration_schema_version = (
        latest_calibration.schema_version if latest_calibration is not None else None
    )
    ...
    latest_calibration_assessment_snapshot_id = (
        latest_calibration.assessment_snapshot_id
        if latest_calibration is not None
        else None
    )
    ...
    return ResearchEvidenceSummaryResponse(
        ...
        most_recent_labeled_outcome_label_method_id=most_recent_labeled_outcome_label_method_id,
        most_recent_labeled_outcome_label_method_version=most_recent_labeled_outcome_label_method_version,
        most_recent_labeled_outcome_label_schema_version=most_recent_labeled_outcome_label_schema_version,
        most_recent_labeled_outcome_label_state=most_recent_labeled_outcome_label_state,
        ...
        latest_outcome_label_method_version=latest_outcome_label_method_version,
        latest_outcome_label_schema_version=latest_outcome_label_schema_version,
        latest_outcome_label_state=latest_outcome_label_state,
        latest_outcome_label_bar_source=latest_outcome_label_bar_source,
        latest_outcome_label_as_of_trading_date=latest_outcome_label_as_of_trading_date,
        ...
        latest_calibration_assessment_snapshot_id=latest_calibration_assessment_snapshot_id,
        detail=(
