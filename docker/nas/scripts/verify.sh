...
print_checklist() {
  ...
  echo " 95. Authenticated evidence-summary includes Phase 199 latest_outcome_label_computed_at (Phase 200)"
  echo " 96. Authenticated evidence-summary includes Phase 201 latest_outcome_label_method_id (Phase 202)"
  echo " 97. Authenticated evidence-summary includes Phase 203 latest_outcome_label_method_version (Phase 204)"
  ...
  echo "124. Authenticated POST /market-data/ingest tip refresh + latest_trading_date (Phase 257-266; unchanged lag OK)"
...
if ! grep -q '"latest_outcome_label_computed_at"' "${summary_body}"; then
  echo "evidence-summary missing latest_outcome_label_computed_at (Phase 199/200)" >&2
  exit 1
fi
echo "OK  Phase 200 latest_outcome_label_computed_at field present"
# Phase 202: latest_outcome_label_method_id from Phase 201 (null OK).
if ! grep -q '"latest_outcome_label_method_id"' "${summary_body}"; then
  echo "evidence-summary missing latest_outcome_label_method_id (Phase 201/202)" >&2
  exit 1
fi
echo "OK  Phase 202 latest_outcome_label_method_id field present"
# Phase 204: latest_outcome_label_method_version from Phase 203 (null OK).
if ! grep -q '"latest_outcome_label_method_version"' "${summary_body}"; then
  echo "evidence-summary missing latest_outcome_label_method_version (Phase 203/204)" >&2
  exit 1
fi
echo "OK  Phase 204 latest_outcome_label_method_version field present"
# Phase 206: latest_outcome_label_schema_version from Phase 205 (null OK).
if ! grep -q '"latest_outcome_label_schema_version"' "${summary_body}"; then
  echo "evidence-summary missing latest_outcome_label_schema_version (Phase 205/206)" >&2
  exit 1
fi
echo "OK  Phase 206 latest_outcome_label_schema_version field present"
# Phase 208: latest_outcome_label_state from Phase 207 (null OK).
if ! grep -q '"latest_outcome_label_state"' "${summary_body}"; then
  echo "evidence-summary missing latest_outcome_label_state (Phase 207/208)" >&2
  exit 1
fi
echo "OK  Phase 208 latest_outcome_label_state field present"
# Phase 210: latest_outcome_label_bar_source from Phase 209 (null OK).
if ! grep -q '"latest_outcome_label_bar_source"' "${summary_body}"; then
  echo "evidence-summary missing latest_outcome_label_bar_source (Phase 209/210)" >&2
  exit 1
fi
...
if ! grep -q '"most_recent_labeled_outcome_label_id"' "${summary_body}"; then
  echo "evidence-summary missing most_recent_labeled_outcome_label_id (Phase 213/214)" >&2
  exit 1
fi
echo "OK  Phase 214 most_recent_labeled_outcome_label_id field present"
# Phase 216: most_recent_labeled_outcome_label_method_id from Phase 215 (null OK when no scan labels).
if ! grep -q '"most_recent_labeled_outcome_label_method_id"' "${summary_body}"; then
  echo "evidence-summary missing most_recent_labeled_outcome_label_method_id (Phase 215/216)" >&2
  exit 1
fi
echo "OK  Phase 216 most_recent_labeled_outcome_label_method_id field present"
# Phase 218: most_recent_labeled_outcome_label_method_version from Phase 217 (null OK when no scan labels).
if ! grep -q '"most_recent_labeled_outcome_label_method_version"' "${summary_body}"; then
  echo "evidence-summary missing most_recent_labeled_outcome_label_method_version (Phase 217/218)" >&2
  exit 1
fi
...
if ! grep -q '"most_recent_labeled_outcome_label_state"' "${summary_body}"; then
  echo "evidence-summary missing most_recent_labeled_outcome_label_state (Phase 221/222)" >&2
  exit 1
fi
echo "OK  Phase 222 most_recent_labeled_outcome_label_state field present"
# Phase 224: most_recent_labeled_outcome_label_bar_source from Phase 223 (null OK when no scan labels).
if ! grep -q '"most_recent_labeled_outcome_label_bar_source"' "${summary_body}"; then
  echo "evidence-summary missing most_recent_labeled_outcome_label_bar_source (Phase 223/224)" >&2
  exit 1
fi
echo "OK  Phase 224 most_recent_labeled_outcome_label_bar_source field present"
# Phase 226: most_recent_labeled_outcome_label_computed_at from Phase 225 (null OK when no scan labels).
if ! grep -q '"most_recent_labeled_outcome_label_computed_at"' "${summary_body}"; then
  echo "evidence-summary missing most_recent_labeled_outcome_label_computed_at (Phase 225/226)" >&2
  exit 1
fi
...
if ! grep -q '"scan_labeled_freshness_lag_trading_days"' "${summary_body}"; then
  echo "evidence-summary missing scan_labeled_freshness_lag_trading_days (Phase 229/230)" >&2
  exit 1
fi
echo "OK  Phase 230 scan_labeled_freshness_lag_trading_days field present"
# Phase 232: latest_assessment_is_label_ready from Phase 231 (null OK when no assessment).
if ! grep -q '"latest_assessment_is_label_ready"' "${summary_body}"; then
  echo "evidence-summary missing latest_assessment_is_label_ready (Phase 231/232)" >&2
  exit 1
fi
...
if ! grep -q '"most_recent_labelable_as_of_trading_date"' "${summary_body}"; then
  echo "evidence-summary missing most_recent_labelable_as_of_trading_date (Phase 235/236)" >&2
  exit 1
fi
echo "OK  Phase 236 most_recent_labelable_as_of_trading_date field present"
# Phase 238: most_recent_unlabeled_labelable_as_of_trading_date from Phase 237 (null OK when none).
if ! grep -q '"most_recent_unlabeled_labelable_as_of_trading_date"' "${summary_body}"; then
  echo "evidence-summary missing most_recent_unlabeled_labelable_as_of_trading_date (Phase 237/238)" >&2
  exit 1
fi
...
if ! grep -q '"most_recent_unlabeled_assessment_id"' "${summary_body}"; then
  echo "evidence-summary missing most_recent_unlabeled_assessment_id (Phase 241/242)" >&2
  exit 1
fi
echo "OK  Phase 242 most_recent_unlabeled_assessment_id field present"
# Phase 244: most_recent_unlabeled_as_of_trading_date from Phase 243 (null OK when none unlabeled).
if ! grep -q '"most_recent_unlabeled_as_of_trading_date"' "${summary_body}"; then
  echo "evidence-summary missing most_recent_unlabeled_as_of_trading_date (Phase 243/244)" >&2
  exit 1
fi
echo "OK  Phase 244 most_recent_unlabeled_as_of_trading_date field present"
# Phase 246: latest_assessment_forward_bar_shortfall from Phase 245 (null OK when N/A; 0 OK when ready).
if ! grep -q '"latest_assessment_forward_bar_shortfall"' "${summary_body}"; then
  echo "evidence-summary missing latest_assessment_forward_bar_shortfall (Phase 245/246)" >&2
  exit 1
fi
...
if ! grep -q '"latest_assessment_last_available_label_bar_date"' "${summary_body}"; then
  echo "evidence-summary missing latest_assessment_last_available_label_bar_date (Phase 249/250)" >&2
  exit 1
fi
echo "OK  Phase 250 latest_assessment_last_available_label_bar_date field present"
# Phase 252: latest_assessment_min_horizon_forward_bar_shortfall from Phase 251 (null/0 OK).
if ! grep -q '"latest_assessment_min_horizon_forward_bar_shortfall"' "${summary_body}"; then
  echo "evidence-summary missing latest_assessment_min_horizon_forward_bar_shortfall (Phase 251/252)" >&2
  exit 1
fi
...
if ! grep -q '"latest_trading_date"' "${ingest_body}"; then
  echo "market-data/ingest missing latest_trading_date (Phase 259/260)" >&2
  exit 1
fi
if ! grep -q '"primary_latest_trading_date"' "${ingest_body}"; then
  echo "market-data/ingest missing primary_latest_trading_date (Phase 263/264)" >&2
  exit 1
fi
echo "OK  Phase 266 ingest HTTP 200 results + primary_latest_trading_date present"
...
if ! grep -q '"latest_outcome_label_computed_at"' "${export_body}"; then
  echo "evidence-summary/export missing latest_outcome_label_computed_at (Phase 199/200)" >&2
  exit 1
fi
if ! grep -q '"latest_outcome_label_method_id"' "${export_body}"; then
  echo "evidence-summary/export missing latest_outcome_label_method_id (Phase 201/202)" >&2
  exit 1
fi
if ! grep -q '"latest_outcome_label_method_version"' "${export_body}"; then
  echo "evidence-summary/export missing latest_outcome_label_method_version (Phase 203/204)" >&2
  exit 1
fi
if ! grep -q '"latest_outcome_label_schema_version"' "${export_body}"; then
  echo "evidence-summary/export missing latest_outcome_label_schema_version (Phase 205/206)" >&2
  exit 1
fi
if ! grep -q '"latest_outcome_label_state"' "${export_body}"; then
  echo "evidence-summary/export missing latest_outcome_label_state (Phase 207/208)" >&2
  exit 1
fi
...
if ! grep -q '"most_recent_labeled_outcome_label_id"' "${export_body}"; then
  echo "evidence-summary/export missing most_recent_labeled_outcome_label_id (Phase 213/214)" >&2
  exit 1
fi
if ! grep -q '"most_recent_labeled_outcome_label_method_id"' "${export_body}"; then
  echo "evidence-summary/export missing most_recent_labeled_outcome_label_method_id (Phase 215/216)" >&2
  exit 1
fi
...
if ! grep -q '"most_recent_labeled_outcome_label_state"' "${export_body}"; then
  echo "evidence-summary/export missing most_recent_labeled_outcome_label_state (Phase 221/222)" >&2
  exit 1
fi
if ! grep -q '"most_recent_labeled_outcome_label_bar_source"' "${export_body}"; then
  echo "evidence-summary/export missing most_recent_labeled_outcome_label_bar_source (Phase 223/224)" >&2
  exit 1
fi
...
if ! grep -q '"scan_labeled_freshness_lag_trading_days"' "${export_body}"; then
  echo "evidence-summary/export missing scan_labeled_freshness_lag_trading_days (Phase 229/230)" >&2
  exit 1
fi
...
if ! grep -q '"most_recent_labelable_as_of_trading_date"' "${export_body}"; then
  echo "evidence-summary/export missing most_recent_labelable_as_of_trading_date (Phase 235/236)" >&2
  exit 1
fi
...
if ! grep -q '"most_recent_unlabeled_assessment_id"' "${export_body}"; then
  echo "evidence-summary/export missing most_recent_unlabeled_assessment_id (Phase 241/242)" >&2
  exit 1
fi
if ! grep -q '"most_recent_unlabeled_as_of_trading_date"' "${export_body}"; then
  echo "evidence-summary/export missing most_recent_unlabeled_as_of_trading_date (Phase 243/244)" >&2
  exit 1
fi
...
if ! grep -q '"latest_assessment_last_available_label_bar_date"' "${export_body}"; then
  echo "evidence-summary/export missing latest_assessment_last_available_label_bar_date (Phase 249/250)" >&2
  exit 1
fi
