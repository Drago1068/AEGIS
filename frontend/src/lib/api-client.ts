...
export interface ResearchEvidenceSummary {
  ...
  /** label_method_id from most_recent_labeled_outcome_label when present; null otherwise. Distinct from latest_outcome_label_method_id. */
  ...
  /** state from most_recent_labeled_outcome_label when present; null otherwise. Distinct from latest_outcome_label_state. */
  most_recent_labeled_outcome_label_state: string | null;
  /** bar_source from most_recent_labeled_outcome_label when present; null otherwise. Distinct from latest_outcome_label_bar_source. */
  most_recent_labeled_outcome_label_bar_source: string | null;
  /** computed_at from most_recent_labeled_outcome_label when present; null otherwise. Distinct from latest_outcome_label_computed_at. */
  most_recent_labeled_outcome_label_computed_at: string | null;
  /** as_of_trading_date from most_recent_labeled_outcome_label when present; null otherwise. Distinct from latest_outcome_label_as_of_trading_date. */
  most_recent_labeled_outcome_label_as_of_trading_date: string | null;
  /**
   * Exchange trading-day lag from most_recent_labeled_outcome_label_as_of_trading_date to
  ...
  /**
   * Trading date that unlocks min-horizon labeling for latest (calendar from as_of).
   * Null when no assessment or no as_of bar. Never invents closes.
   */
  latest_assessment_min_horizon_required_label_end_date: string | null;
  /**
   * Sessions the label-source tip lags the prior completed exchange session.
  ...
  /** computed_at from latest_outcome_label when latest assessment is labeled; null otherwise. Distinct from assessment/calibration computed_at. */
