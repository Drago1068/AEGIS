# NAS Live Verification Checklist (Phase 17 + Phase 21)
...
(Phases 18–20 history routes). Authoritative scripted checks:
`docker/nas/scripts/verify.ps1` / `verify.sh`.
...
| 2 | `GET {API}/ready` | 200 |
| 3 | Unauthenticated watchlist / daily-bars / research latest / **calibration-readiness** | 401 |
...
| 9 | SSH `alembic current` (when SSH configured) | includes **`0008`** or `head` |
| 10 | TLS (if enabled) | HTTPS URLs + `AEGIS_SESSION_COOKIE_SECURE=true` |
