...
function Write-VerifyChecklist {
    ...
    Write-Host "129. Authenticated evidence-summary Phase 318 partial-label upgrade callout field bundle (UI unit-tested)"
    Write-Host "130. Authenticated outcome-label history Phase 320 horizon coverage badge (UI unit-tested)"
    Write-Host "131. Authenticated evidence-summary Phase 322 min-horizon unlock ready-horizons CTA (UI unit-tested)"
    ...
    Write-Host "136. Authenticated evidence-summary Phase 332 unlabeled-empty ready-horizon CTA (UI unit-tested)"
    Write-Host "137. Authenticated evidence-summary Phase 334 tip-not-ready ready-horizon CTA (UI unit-tested)"
    Write-Host "138. Authenticated evidence-summary Phase 336 partial-labeled upgrade CTA (UI unit-tested)"
    Write-Host "139. Authenticated evidence-summary includes Phase 235 most_recent_labelable_as_of_trading_date (Phase 236)"
    Write-Host "140. Authenticated evidence-summary includes Phase 237 most_recent_unlabeled_labelable_as_of_trading_date (Phase 238)"
    Write-Host "141. Authenticated evidence-summary includes Phase 239 scan_unlabeled_label_ready_count (Phase 240)"
    Write-Host "142. Authenticated evidence-summary includes Phase 241 most_recent_unlabeled_assessment_id (Phase 242)"
    Write-Host "143. Authenticated evidence-summary includes Phase 243 most_recent_unlabeled_as_of_trading_date (Phase 244)"
    Write-Host "144. Authenticated evidence-summary includes Phase 245 latest_assessment_forward_bar_shortfall (Phase 246)"
    Write-Host "145. Authenticated evidence-summary includes Phase 247 latest_assessment_required_label_end_date (Phase 248)"
    Write-Host "146. Authenticated evidence-summary includes Phase 249 latest_assessment_last_available_label_bar_date (Phase 250)"
    Write-Host "147. Authenticated evidence-summary includes Phase 251 latest_assessment_min_horizon_forward_bar_shortfall (Phase 252)"
    Write-Host "148. Authenticated evidence-summary includes Phase 253 latest_assessment_min_horizon_required_label_end_date (Phase 254)"
    Write-Host "149. Authenticated evidence-summary includes Phase 255 stored_bar_calendar_lag_trading_days (Phase 256)"
    Write-Host "150. Authenticated evidence-summary includes Phase 279 latest_primary_fetch_fallback (Phase 280)"
    Write-Host "151. Authenticated evidence-summary Phase 296 primary fetch-fallback callout field bundle (UI unit-tested)"
    Write-Host "152. Authenticated GET daily-bars includes Phase 281 fetch_fallback (Phase 282)"
    Write-Host "153. Authenticated POST /market-data/ingest tip refresh + latest_trading_date (Phase 257-266; unchanged lag OK)"
    Write-Host "154. TLS profile: https:// URLs + Secure cookies when enabled"
}
...
$readyHorizonsUnauthUrl = "$api/research/$verifySymbol/assessments/1/outcome-labels/ready-horizons"
$readyHorizonsUnauthCode = & curl.exe -sS @curlInsecure -o NUL -w "%{http_code}" --max-time 30 `
...
try {
    ...
    try {
        ...
        # Phase 306: UI defaults to distinct-as_of list (ADR-0306); count unique valid as_of.
        $distinctAsOfSet = @{}
        foreach ($row in @($assessBody)) {
            if ($null -eq $row) { continue }
            $asOf = $null
            ...
            if ([string]::IsNullOrWhiteSpace($asOf) -or $asOf -notmatch '^\d{4}-\d{2}-\d{2}$') { continue }
            $distinctAsOfSet[$asOf] = $true
        }
        $distinctAsOf = $distinctAsOfSet.Keys.Count
        $listTotal = @($assessBody).Count
        $hiddenDup = [Math]::Max(0, $listTotal - $distinctAsOf)
        Write-Host "OK  Phase 306 distinct-as_of history list_count=$listTotal distinct_as_of=$distinctAsOf hidden_duplicates=$hiddenDup (UI unit-tested; default distinct)"
    } finally {
    ...
    try {
        ...
        Write-Host "OK  Phase 314 full-horizon backfill uses complete-horizon skip (partial ready-horizons remain upgrade-eligible; unit-tested)"
    ...
    # Phase 310: ready-horizons opt-in path (ADR-0310/0311). 200 with subset OK; 422 fail-closed OK.
    $readyHorizonsUrl = "$api/research/$verifySymbol/assessments/$historyAssessmentId/outcome-labels/ready-horizons"
    $readyHorizonsPath = Join-Path ([System.IO.Path]::GetTempPath()) ("aegis-nas-verify-{0}.ready-horizons.json" -f [guid]::NewGuid().ToString("N"))
    try {
        $readyHorizonsCode = & curl.exe -sS @curlInsecure -o $readyHorizonsPath -w "%{http_code}" --max-time 60 `
            -b $cookieJar -H "Accept: application/json" -X POST $readyHorizonsUrl
        if ($LASTEXITCODE -ne 0) { throw "POST outcome-labels/ready-horizons failed (curl exit $LASTEXITCODE)" }
        Assert-Status -Label "POST $readyHorizonsUrl (auth)" -Actual ([int]$readyHorizonsCode) -Expected @(200, 422)
        $readyHorizonsBody = $null
        if (Test-Path -LiteralPath $readyHorizonsPath) {
            $readyRaw = Get-Content -LiteralPath $readyHorizonsPath -Raw
            if (-not [string]::IsNullOrWhiteSpace($readyRaw)) {
                $readyHorizonsBody = $readyRaw | ConvertFrom-Json
            }
        }
        if ([int]$readyHorizonsCode -eq 200) {
            if ($null -eq $readyHorizonsBody -or $null -eq $readyHorizonsBody.labels) {
                throw "outcome-labels/ready-horizons 200 missing labels object"
            }
            $horizonKeys = @($readyHorizonsBody.labels.PSObject.Properties.Name)
            if ($horizonKeys.Count -lt 1) {
                throw "outcome-labels/ready-horizons 200 returned empty labels"
            }
            Write-Host "OK  Phase 310 ready-horizons status=200 assessment_id=$historyAssessmentId horizons=$($horizonKeys -join ',') (research-only subset OK)"
        } else {
            $reasonPart = "null"
            if ($null -ne $readyHorizonsBody -and $null -ne $readyHorizonsBody.detail -and $null -ne $readyHorizonsBody.detail.reason) {
                $reasonPart = [string]$readyHorizonsBody.detail.reason
            }
            Write-Host "OK  Phase 310 ready-horizons status=422 assessment_id=$historyAssessmentId reason=$reasonPart (fail-closed; tip may still be blocked)"
        }
    } finally {
        if (Test-Path -LiteralPath $readyHorizonsPath) {
    ...
    try {
        ...
        # Phase 326: CTA on mixed-unlabeled callout pointing at Backfill outcome labels (ADR-0326 / ADR-0327).
        Write-Host "OK  Phase 326 mixed-unlabeled backlog CTA fields mixed_unlabeled=$mixedUnlabeled (UI unit-tested; count>0 elevates use_toolbar=Backfill outcome labels)"
        ...
        # Phase 318: UI callout when partial_labeled > 0 (ADR-0318 / ADR-0319).
        Write-Host "OK  Phase 318 partial-label upgrade callout fields partial=$partialScan complete=$completeScan labeled=$labeledScan (UI unit-tested; count>0 elevates callout)"
        # Phase 336: CTA on partial-labeled callout pointing at Backfill outcome labels (ADR-0336 / ADR-0337).
        Write-Host "OK  Phase 336 partial-labeled upgrade CTA fields partial=$partialScan (UI unit-tested; count>0 elevates use_toolbar=Backfill outcome labels)"
        # Phase 320: outcome-label history horizon coverage badge (ADR-0320 / ADR-0321); UI unit-tested.
        Write-Host "OK  Phase 320 outcome-label horizon coverage badge (complete/partial from labels keys; UI unit-tested)"
        ...
        # Phase 286: UI callout uses lag + as_of fields when lag > 0 (ADR-0286).
        $labeledAsOfFresh = $null
        if ($summary.PSObject.Properties.Name -contains "most_recent_labeled_outcome_label_as_of_trading_date") {
            $labeledAsOfFresh = $summary.most_recent_labeled_outcome_label_as_of_trading_date
        }
        $labeledAsOfFreshPart = if ($null -eq $labeledAsOfFresh -or $labeledAsOfFresh -eq "") { "null" } else { [string]$labeledAsOfFresh }
        $tipAsOfFresh = $null
        if ($summary.PSObject.Properties.Name -contains "latest_as_of_trading_date") {
            $tipAsOfFresh = $summary.latest_as_of_trading_date
        }
        $tipAsOfFreshPart = if ($null -eq $tipAsOfFresh -or $tipAsOfFresh -eq "") { "null" } else { [string]$tipAsOfFresh }
        Write-Host "OK  Phase 286 labeled freshness-lag callout fields lag=$scanLagPart labeled_as_of=$labeledAsOfFreshPart tip_as_of=$tipAsOfFreshPart (UI unit-tested; lag>0 elevates callout)"
        # Phase 328: CTA on freshness-lag callout pointing at Backfill ready-horizon labels (ADR-0328 / ADR-0329).
        Write-Host "OK  Phase 328 freshness-lag CTA fields lag=$scanLagPart (UI unit-tested; lag>0 elevates use_toolbar=Backfill ready-horizon labels)"
        ...
        # Phase 334: CTA on tip-not-ready callout pointing at Compute ready-horizon labels (ADR-0334 / ADR-0335).
        Write-Host "OK  Phase 334 tip-not-ready CTA fields tip_ready=$labelReadyPart forward_shortfall=$fwdShortfallPart (UI unit-tested; tip_not_ready elevates use_toolbar=Compute ready-horizon labels)"
        ...
        # Phase 288: UI callout when count=0 and unlabeled assessment exists (ADR-0288).
        $unlabeledId = $null
        if ($summary.PSObject.Properties.Name -contains "most_recent_unlabeled_assessment_id") {
            $unlabeledId = $summary.most_recent_unlabeled_assessment_id
        }
        $unlabeledIdPart = if ($null -eq $unlabeledId -or $unlabeledId -eq "") { "null" } else { [string]$unlabeledId }
        $unlabeledAsOfPart = "null"
        if ($summary.PSObject.Properties.Name -contains "most_recent_unlabeled_as_of_trading_date") {
            $ua = $summary.most_recent_unlabeled_as_of_trading_date
            if ($null -ne $ua -and $ua -ne "") { $unlabeledAsOfPart = [string]$ua }
        }
        Write-Host "OK  Phase 288 unlabeled label-ready empty callout fields ready_count=$unlabeledReadyCount unlabeled_id=$unlabeledIdPart unlabeled_as_of=$unlabeledAsOfPart (UI unit-tested; count=0+id elevates callout)"
        # Phase 332: CTA on unlabeled-empty callout pointing at Backfill ready-horizon labels (ADR-0332 / ADR-0333).
        Write-Host "OK  Phase 332 unlabeled-empty CTA fields ready_count=$unlabeledReadyCount unlabeled_id=$unlabeledIdPart (UI unit-tested; count=0+id elevates use_toolbar=Backfill ready-horizon labels)"
        # Phase 290: UI groups tip/freshness/unlabeled-empty callouts (ADR-0290); field bundles above are the evidence.
        $diagParts = @()
        ...
        if ($summary.PSObject.Properties.Name -contains "partial_labeled_assessment_count") {
            $pl = $summary.partial_labeled_assessment_count
            if ($null -ne $pl -and [int]$pl -gt 0) { $diagParts += "partial_labeled_upgrade" }
        }
        if (
            ($labelReadyPart -eq "False") -and
            ($summary.PSObject.Properties.Name -contains "latest_assessment_min_horizon_forward_bar_shortfall")
        ) {
            $mh = $summary.latest_assessment_min_horizon_forward_bar_shortfall
            if ($null -ne $mh -and $mh -ne "" -and [int]$mh -eq 0) {
                $diagParts += "min_horizon_unlock"
            }
        }
        if (
            ($labelReadyPart -eq "True") -and
            ($summary.PSObject.Properties.Name -contains "latest_outcome_label_id")
        ) {
            $tipLabelId = $summary.latest_outcome_label_id
            if ($null -eq $tipLabelId -or $tipLabelId -eq "") {
                $diagParts += "full_horizon_unlock"
            }
        }
        ...
        # Phase 294: disclosure open-by-default when any trigger (ADR-0294); UI unit-tested.
        Write-Host "OK  Phase 294 labeling diagnostics disclosure open-by-default triggers=$diagJoined (UI unit-tested)"
        # Phase 298: summary active-count badge from same triggers (ADR-0298); UI unit-tested.
        Write-Host "OK  Phase 298 labeling diagnostics summary active_count=$($diagParts.Count) triggers=$diagJoined (UI unit-tested)"
        ...
        # Phase 244: most_recent_unlabeled_as_of_trading_date from Phase 243 (null OK when none unlabeled).
        if (-not ($summary.PSObject.Properties.Name -contains "most_recent_unlabeled_as_of_trading_date")) {
            throw "evidence-summary missing most_recent_unlabeled_as_of_trading_date (Phase 243/244)"
        }
        ...
        # Phase 250: latest_assessment_last_available_label_bar_date from Phase 249 (null OK when N/A).
        if (-not ($summary.PSObject.Properties.Name -contains "latest_assessment_last_available_label_bar_date")) {
            throw "evidence-summary missing latest_assessment_last_available_label_bar_date (Phase 249/250)"
        }
        $lastAvail = $summary.latest_assessment_last_available_label_bar_date
        $lastAvailPart = if ($null -eq $lastAvail -or $lastAvail -eq "") { "null" } else { [string]$lastAvail }
        Write-Host "OK  Phase 250 latest_assessment_last_available_label_bar_date=$lastAvailPart"
        # Phase 252: latest_assessment_min_horizon_forward_bar_shortfall from Phase 251 (null/0 OK).
        if (-not ($summary.PSObject.Properties.Name -contains "latest_assessment_min_horizon_forward_bar_shortfall")) {
            throw "evidence-summary missing latest_assessment_min_horizon_forward_bar_shortfall (Phase 251/252)"
        }
        ...
        # Phase 322: UI CTA when min-horizon shortfall=0 and tip not fully ready (ADR-0322 / ADR-0323).
        Write-Host "OK  Phase 322 min-horizon unlock CTA fields min_shortfall=$minShortfallPart tip_ready=$labelReadyPart min_end=$minEndPart (UI unit-tested; shortfall=0+tip_not_ready elevates callout)"
        # Phase 324: UI CTA when tip fully label-ready but unlabeled (ADR-0324 / ADR-0325).
        $tipOutcomeLabelIdPart = "null"
        if ($summary.PSObject.Properties.Name -contains "latest_outcome_label_id") {
            $tol = $summary.latest_outcome_label_id
            if ($null -ne $tol -and $tol -ne "") { $tipOutcomeLabelIdPart = [string]$tol }
        }
        Write-Host "OK  Phase 324 full-horizon unlock CTA fields tip_ready=$labelReadyPart tip_outcome_label_id=$tipOutcomeLabelIdPart forward_shortfall=$forwardShortfallPart (UI unit-tested; tip_ready+unlabeled elevates callout)"
        ...
        if ($summary.PSObject.Properties.Name -contains "latest_assessment_is_label_ready") {
            $tr = $summary.latest_assessment_is_label_ready
            if ($null -ne $tr) { $tipReadyPart = [string]$tr }
        }
        Write-Host "OK  Phase 308 labeling frontier readout tip_ready=$tipReadyPart forward_shortfall=$forwardShortfallPart required_end=$requiredEndPart min_horizon_shortfall=$minShortfallPart min_horizon_end=$minEndPart last_available=$lastAvailPart (UI unit-tested; existing fields only)"
        # Phase 282: daily-bars tip fetch_fallback from stored raw_payload (ADR-0282).
        $barsPath = Join-Path ([System.IO.Path]::GetTempPath()) ("aegis-nas-verify-{0}.daily-bars.json" -f [guid]::NewGuid().ToString("N"))
        try {
            $barsCode = & curl.exe -sS @curlInsecure -o $barsPath -w "%{http_code}" --max-time 60 `
        ...
        try {
            ...
            if ($null -ne $verifyRow) {
                $providerTip = $verifyRow.latest_trading_date
                if (-not ($verifyRow.PSObject.Properties.Name -contains "latest_trading_date")) {
    ...
    try {
        ...
        if (-not ($exportBody.PSObject.Properties.Name -contains "latest_assessment_last_available_label_bar_date")) {
            throw "evidence-summary/export missing latest_assessment_last_available_label_bar_date (Phase 249/250)"
        }
